...
//
    ...
    @Query(value="select s.company,date,s.supplierName,so.status,so.total,so.supplierOrderId,s.supplierNic,s.supplierId from Supplier s, SupplierOrder so where s.supplierNic=so.supplier.supplierNic and so.supplierOrderId=:supplierOrderId")
    ...
    @Query(value = "select s.company,date,s.supplierName,so.status,so.total,so.supplierOrderId from Supplier s, SupplierOrder so where s.supplierNic=so.supplier.supplierNic and so.supplierOrderId=:supplierOrderId and so.status='false'")
